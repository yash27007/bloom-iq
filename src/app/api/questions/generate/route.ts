  ...
  try {
    ...
    const course = await prisma.course.findFirst({
      where: {
        id: courseId,
        OR: [
          { courseCoordinatorId: session.user.id },
          { moduleCoordinatorId: session.user.id },
          { programCoordinatorId: session.user.id },
        ],
      },
    });
    ...
    // For development, simulate immediate processing
    if (process.env.NODE_ENV === "development") {
      // Start background processing immediately without blocking the response
      processQuestionGeneration(job.id, {
        courseId,
        materialId,
        unit,
        questionTypes,
      ...
      // In production, trigger Inngest function
      try {
        ...
        // Fall back to local processing even in production if Inngest fails
        processQuestionGeneration(job.id, {
          courseId,
          materialId,
          unit,
