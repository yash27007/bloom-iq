...
import { vectorRAG } from "./supabase-vector-rag";
    ...
    try {
      ...
      const course = await prisma.course.findUnique({
        ...
        select: {
          ...
          courseName: true,
          courseMaterials: {
            where: request.unit ? { unit: request.unit } : {},
...
Generate exactly ${request.count} questions now:`;
    ...
    try {
      ...
      return questions.map((q: any) => ({
        ...
        metadata: {
          ...
          estimatedTime:
            q.metadata?.estimatedTime || timeMap[marksValue] || "10-15 minutes",
  ...
  /**
   * Generate questions for a complete question paper pattern
   */
  async generateQuestionPaper(
    ...
    try {
      console.log(
        `📄 Generating complete question paper for course: ${courseId}`
      );
      ...
      if (!pattern) {
        throw new Error("Question paper pattern not found");
      }

      const partA: Array<any> = [];
      const partB: Array<any> = [];
      let totalMarks = 0;
      const bloomDistribution: Record<string, number> = {};
      const unitCoverage: Set<number> = new Set();
      ...
      for (const slot of pattern.slots) {
        ...
        const questions = await this.generateContextualQuestions({
          ...
          unit: preferences?.units
            ? preferences.units[
                Math.floor(Math.random() * preferences.units.length)
              ]
            : undefined,
          marks: slot.marks,
        ...
        const slotQuestions = {
          ...
          hasORQuestions: slot.hasORQuestions,
          questions: questions,
          instructions: `Answer ${slot.hasORQuestions ? "any" : "all"} ${
            slot.questionCount
          } question${
            slot.questionCount > 1 ? "s" : ""
          }. Each question carries ${slot.marks} marks.`,
        };

        if (slot.marks === "TWO") {
          partA.push(slotQuestions);
        } else {
          partB.push(slotQuestions);
        }

        // Update metadata
        totalMarks += parseInt(slot.marks.toString()) * slot.questionCount;
        questions.forEach((q) => {
          bloomDistribution[q.bloomLevel] =
            (bloomDistribution[q.bloomLevel] || 0) + 1;
          if (q.unit) unitCoverage.add(q.unit);
        });
      }
      ...
      return {
        ...
        metadata: {
          ...
          bloomDistribution,
          unitCoverage: Array.from(unitCoverage).sort(),
        },
      };
    } catch (error) {
